	...
	<body>
		...
		<caption><h2>申请退货的订单</h2></caption>
			<tr class="firstrow">
				...
				<td style="width:15%">申请退货时间</td>
				<td style="width:6%">订单详情</td>
				<td style="width:6%">退货</td>
				<td style="width:10%">取消退货申请</td>
		</tr>
...
<?php
	...
	/* 查询数据库 */
	$sql1 = "SELECT * FROM orderall WHERE status = '3'";
	...
	while ($row1 = mysqli_fetch_array($result1)) {
		echo '<tr>';
			...
			echo '</form>';
			echo '<form action="returnConfirm.php" method="post">';
				echo '<input name="OrderNo" value=' . $row1['OrderNo'] . '   type="hidden">';
				echo '<td>'.'<input name="submit" type="submit" value="确认">'.'</td>';
			echo '</form>';
			echo '<form action="returnFail.php" method="post">';
					echo '<input name="OrderNo" value=' . $row1['OrderNo'] . '   type="hidden">';
					echo '<td>'.'<input name="submit" type="submit" value="确认">'.'</td>';
				echo '</form>';
			echo '</tr>';
