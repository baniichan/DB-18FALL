...
<html>
	...
	<body>
		...
		<table id = "table">
		<caption><h2><?php echo $typename; ?></h2></caption>
			<tr class="firstrow">
...
<?php
    ...
    $sql=mysqli_query($conn,"SELECT * FROM bookinfo WHERE type=$type ");	// 查询bookinfo中的记录数
	...
	/* 显示图书记录 */
	$sql =  "SELECT * FROM bookinfo WHERE type=$type limit " . $offset . ',' . $num;	// 查询book
...
?>
	...
	<!-- 显示分页 -->
	<center><p>
	<form name="page" method="get" action="type.php?type='诗歌'" onSubmit="return chk(this)">
    	...
    	<a href="type.php?page=<?=$prev;?>">上一页</a>&nbsp;&nbsp;&nbsp;
    	<a href="type.php?page=<?=$next;?>">下一页</a>&nbsp;&nbsp;&nbsp;
    	<a href="type.php?page=<?=$pageCount;?>">尾页</a>
	</form>
	</p></center>
